...
### 1.2 Key Features
- User-configurable inputs: Location (e.g., Seattle, WA), positions (dropdown: SDE, AI Engineer, etc.), target companies with company career page pre-filtered urls, resume upload, Telegram integration.
- Timed scanning: Weekdays, every 2 hours (e.g., 4x/day), fetching only new jobs from hardcoded or user-provided filtered career URLs.
- AI Matching: Vector embeddings for semantic similarity, RAG-enhanced LLM for explanations (reducing hallucinations via grounded retrieval).
...
- Support 20-50 target companies with minimal maintenance (hardcoded filtered URLs).
...
- **AI Pipeline**: CrewAI multi-agent framework:
  - **Fetcher Agent**: Scrapes career pages using Playwright based on user input pre-filtered career webpage urls (stealth mode, pagination handling). Extracts: job_id, job_url, title, company, location, description, salary (optional), posted_date (optional).
  - **Matcher Agent**: Coarse filters (keywords for location/position), then vector embedding similarity (FAISS). Computes match score (0-1 scale).
...
- **Storage**: SQLite for push history, seen jobs (deduplication via job_id/URL).
...
1. User configures inputs via Streamlit (location, companies urls, resume, Telegram chat_id).
2. Resume uploaded → Parsed/extracted → Embedded → Stored in FAISS index.
3. Manual scan triggered → Fetcher gets jobs from filtered URLs → Extracts job_id, URL, title, company, location, description, salary (optional), posted_date (optional).
4. Matcher checks push_history DB (via job_id) → Skips duplicates → Applies filters → Computes embedding similarity (match_score 0-1).
5. High matches (e.g., similarity > 0.7) → Analyzer generates RAG-based explanation ("Why this fits" + "Suggest emphasizing").
6. Notifier formats message → Pushes to Telegram (one message per job, 1-2 sec delay) → Logs to DB.
...
- **user_config**: id (PK), location (e.g., "Seattle, WA"), positions (JSON array), companies (JSON array with filtered_urls), resume_path, telegram_chat_id.
- **push_history**: id (PK), company, job_id (unique per company), job_url, title, location, salary (TEXT, nullable), posted_date (DATETIME, nullable), match_score (FLOAT, 0-1), pushed_at (DATETIME), explanation (TEXT, RAG-generated insights), status (e.g., 'pushed', 'archived').
...
- **POST /configure**: Update user configs (location, positions, companies, telegram_chat_id).
- **POST /scan**: Manual/trigger scan (async).
- **GET /history**: Fetch push history (paginated).
- **GET /health**: System status.

Note: Resume upload is handled via FastAPI endpoint, while configuration and dashboard viewing use Streamlit UI.
...
### 10.1 Risks
- Career page changes: Mitigate with user-editable URLs; fallback to search tools.
- LLM Hallucinations: RAG grounding + prompt engineering; test with real resume/job pairs.
- Rate Limits/Bans: 
  - Scraping: Stealth mode + proxies; monitor error logs.
